...
export function getAllSpriteUrls(): string[] {
  return SPRITE_CHARACTERS.flatMap((character) => EXPRESSIONS.map((expression) => getSpritePath(character, expression)));
}
...
export const CRITICAL_PRELOAD_URLS = [
  ...
  ...SPRITE_CHARACTERS.map((character) => getSpritePath(character, 'neutral')),
];
